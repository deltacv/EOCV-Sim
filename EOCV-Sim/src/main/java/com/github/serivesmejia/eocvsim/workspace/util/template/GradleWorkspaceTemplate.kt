...
object GradleWorkspaceTemplate : WorkspaceTemplate() {
    ...
    private fun reformatTemplate(folder: File) {
        ...
        // based on the generated build-info JSON loaded by BuildInfo
        val fileContents = SysUtil.loadFileStr(buildGradleFile)
            .replace("\$eocvsim_version", BuildInfo.STANDARD_VERSION_STRING)
            .replace("\$opencv_version", BuildInfo.OPENCV_VERSION)
