...
object PluginSignatureVerifier {
    ...
    fun verify(pluginFile: File): PluginSignature {
        ...
        ZipFile(pluginFile).use { zip ->
            ...
            // Read the signature TOML from the entry as an InputStream so it matches
            // the Toml.read(InputStream) overload (avoid BufferedReader which is not supported)
            // force the Toml type so Kotlin sees the helper API
            val signature: Toml = zip.getInputStream(signatureEntry).use { Toml().read(it) }
            ...
            val signatureData: Toml = signature.getTable("signatures") ?: run {
            ...
            // Convert table to a typed map so Kotlin can destructure entries reliably
            val signatureMap = signatureData.toMap() as Map<String, Any>
            for ((path, signAny) in signatureMap) {
                if (signAny !is String) {
                    logger.warn("Signature for class $path is not a string")
                    return emptyResult
                }

                signatures[path] = signAny
            }
